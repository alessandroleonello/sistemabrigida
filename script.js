    ...
    mainAppContainer.addEventListener('click', (e) => {
        ...
        if (printPendingBillsBtn) {
            ...
            billsToPay.forEach(bill => {
                ...
                let showRow = true;
                if (currentBillMetricFilter === 'topay' && bill.tipo !== 'Saída') showRow = false;
...
function renderBillsTab(entries) {
    ...
    if (billsToPay.length === 0) {
        ...
        billsToPay.forEach(bill => {
            ...
            if (bill.vencimento) {
                ...
                if (diffDays >= 0 && diffDays <= 10) totalSoonSum += bill.valor;
            }

            totalToPaySum += bill.valor;
            if (bill.tipo === 'Entrada') {
                totalReceivableSum += bill.valor;
            } else if (bill.tipo === 'Saída') {
                totalToPayMetricSum += bill.valor;
            }
    ...
    const metricToPayEl = document.getElementById('metric-bills-topay');
    ...
    if (metricToPayEl) metricToPayEl.textContent = `R$ ${totalToPayMetricSum.toFixed(2).replace('.', ',')}`;
...
const cardFilterToPay = document.getElementById('card-filter-topay');
...
function updateBillMetricCardsUI() {
    ...
    [cardFilterToPay, cardFilterSoon, cardFilterAll, cardFilterReceivable].forEach(card => {
        if (card) {
    ...
    if (currentBillMetricFilter === 'topay') activeCard = cardFilterToPay;
...
if (cardFilterToPay) cardFilterToPay.addEventListener('click', () => toggleMetricFilter('topay'));
...
if (cardSummaryLateBills) {
    cardSummaryLateBills.addEventListener('click', () => {
        document.querySelector('.financeiro-tab-btn[data-tab="tab-financeiro-bills"]')?.click();
        currentBillMetricFilter = 'topay'; // Ativa automaticamente o filtro A Pagar
        updateBillMetricCardsUI();
        if (allFinancialEntries) renderBillsTab(allFinancialEntries);
    });
}
